...
public class WebServer {
    public static void main(String[] args) {
        ...
        //ThreadPool
        Sender<Processor> sender = new Sender<>();
        WorkerWorkerThreadPool<Processor> workerThreadPool;


        for(int i=0; i<numOfThreads; i++){
            workerThreadPool = new WorkerWorkerThreadPool<>(i,sender);
        ...
        try (ServerSocket serverSocket = new ServerSocket(port, queueLength)) {
            ...
            while (true) {
                ...
                try {
                    ...
                    sender.add(proc);
                  // Multi-thread
//                    WorkerThread workerThread = new WorkerThread(id++,proc);
//                    workerThread.start();
